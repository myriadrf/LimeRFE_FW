...
void loop() {
  if (Serial.available()) {
    ...
    if (activeBuffer[0] == CMD_HELLO){
      ...
      if (command == CMD_MODE || command == CMD_FAN) {       // cmd_mode and cmd_fan are shorter
...
unsigned char configureState(unsigned char rxChannelID, unsigned char txChannelId, unsigned char rxConn, unsigned char txConn, unsigned char notch, unsigned char attenuation, unsigned char swrEn, unsigned char swrSrc){
  switch(rxConn){
      case CONN_DEFAULT:{
        if (rxChannelID == CID_HAM_0030 || rxChannelID == CID_HAM_0070){
          rxConn = CONN_30;
        } else {
          rxConn = CONN_TXRX;
        }
  ...
  switch(txConn){
      case CONN_DEFAULT:{
        if (txChannelId == CID_HAM_0030 || txChannelId == CID_HAM_0070){
          txConn = CONN_30;
        } else {
          txConn = CONN_TXRX;
        }
      ...
      case CONN_TX:{
        if (txChannelId == CID_HAM_0030 || txChannelId == CID_HAM_0070 || isCell(txChannelId)){
          // error
          return ERROR_TX_CONN;
        }
        break;
  ...
  switch(rxChannelID){
    ...
    case CID_HAM_0070:  {
        memcpy(nextState, STATE_HAM70, sizeof(STATE_HAM70[0])*CHAIN_SIZE);
        break;
      }
    case CID_HAM_0145:  {
    ...
    case CID_HAM_0220:  {
        memcpy(nextState, STATE_HAM220, sizeof(STATE_HAM220[0])*CHAIN_SIZE);
        break;
      }
    case CID_HAM_0435:  {
    ...
    case CID_HAM_0920:  {
        memcpy(nextState, STATE_HAM920, sizeof(STATE_HAM920[0])*CHAIN_SIZE);
        break;
      }
    case CID_HAM_1280:  {
  ...
  if(isCell(rxChannelID) || isCell(txChannelId)) {       
      ...
      switch(txChannelId){
        ...
        case CID_HAM_0070:  {
            setTxState(nextState, STATE_HAM70);
            break;
          }
        case CID_HAM_0145:  {
        ...
        case CID_HAM_0220:  {
            setTxState(nextState, STATE_HAM220);
            break;
          }
        case CID_HAM_0435:  {
        ...
        case CID_HAM_0920:  {
            setTxState(nextState, STATE_HAM920);
            break;
          }
        case CID_HAM_1280:  {
  ...
  if(rxChannelID != CID_NOT_SELECTED){
    ...
    if (isCell(rxChannelID)) {                      /////////////////// CELL ////////////////
      ...
      if (rxChannelID == CID_CELL_BAND38){  
        if (mode == RFE_MODE_TX){                                // TX mode Band 38
          ...
          if (rxChannelID == CID_CELL_BAND07){
              iBAND38TX = 1;                                     // enable PA for band 7 (the same pa as for band 38)
          }
      }                                                          /////////////////// CELL /////////////////
      ...
      switch(mode){
        ...
        case RFE_MODE_TX: {
          if (txChannelID == CID_HAM_0030 || txChannelID == CID_HAM_0070) {                    // 30MHz, 70MHz
            iRELAY = 1;
          } else {
            iTXRX0 = 1;
            iTXRX1 = 1;
        ...
        case RFE_MODE_RX:{
          if (rxChannelID == CID_HAM_0030 || rxChannelID == CID_HAM_0070) {                    // 30MHz, 70MHz
        ...
        case RFE_MODE_TXRX: {
          if(activeTxConn != activeRxConn){                                     // for RXTX rx_conn and tx_conn have to be different        
            if (rxChannelID == CID_HAM_0030 || rxChannelID == CID_HAM_0070) {   // RX_30MHz, RX_70MHz
              if (activeRxConn == CONN_TXRX){
                  iTXRX0 = 1;
                  iTXRX1 = 0;
              } else if(activeRxConn == CONN_30){
              ...
              iTXRX0 = 1;
              iTXRX1 = 0;
            } else if (isRxH(rxChannelID)) {                    // RX_H
              iTXRX0 = 0;
              iTXRX1 = 0;
            }
            
            if (txChannelID == CID_HAM_0030 || txChannelID == CID_HAM_0070) {                  // TX_30MHz, TX_70MHz
              iRELAY = 1;
